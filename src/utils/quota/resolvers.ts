...
const getCodexCandidateRecords = (file: AuthFileItem): Array<Record<string, unknown> | null> => {
  ...
  const fileIdTokenClaims = parseIdTokenPayload(file.id_token);
  const metadataIdTokenClaims = parseIdTokenPayload(metadata?.id_token);
  const attributesIdTokenClaims = parseIdTokenPayload(attributes?.id_token);
